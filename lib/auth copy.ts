    ...
    Credentials({
      ...
      async authorize(credentials: any) {
        ...
        try {
          ...
          if (data) {
            return data; // Return user data if login successful
          } else {
            throw new Error(message); // Throw error message if login failed
          }
        } catch (error) {
          throw new Error(error.message); // Throw error if any unexpected error occurs
        }
      }
      // async authorize(credentials: any) {
      //   const { email, password } = credentials as {
